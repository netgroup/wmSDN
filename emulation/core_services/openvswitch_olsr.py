...
class OpenvswitchService(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        if filename == cls._configs[0]: # start
                return cfg + cls.generateOpenvswitchConf(node, services, openvswitch_dir, olsr_dir, start=True)
        elif filename == cls._configs[1]: # stop
                return cfg + cls.generateOpenvswitchConf(node, services, openvswitch_dir, olsr_dir, start=False)
        elif filename == cls._configs[2]: # env
                return cls.generateOpenvswitchEnv(node, services, openvswitch_dir, olsr_dir)
        else:
                raise ValueError
    
    @classmethod
    def generateOpenvswitchEnv(cls, node, services, openvswitch_dir, olsr_dir):
            cfg = """
...
""" % (openvswitch_dir, olsr_dir)
            return cfg

    @classmethod
    def generateOpenvswitchConf(cls, node, services, openvswitch_dir, olsr_dir, start):
...
start() {
    ...
    printandexec ln -s $OLSR_DIR/openvswitch/o2o.sh .
    printandexec ln -s $OLSR_DIR/openvswitch/emergency_flows.sh .
...
""" % (openvswitch_dir, olsr_dir, nodename2dpid(node.name))
